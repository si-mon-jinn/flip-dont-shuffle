#!/usr/bin/env python3
"""Generate/update LaTeX table for perplexity comparison (Selfhash columns).

Updates tab_ppl.tex with selfhash vs gpu-selfhash data.
"""
...
def load_perplexity(scheme: str, gamma: float, delta: float):
    prefix = "roc500_gselfhash" if scheme == "gpu" else "roc500_selfhash"
    exp_name = f"roc500_{'g' if scheme == 'gpu' else ''}selfhash_d{int(delta)}_g{int(gamma*100)}"
...
def main():
    ...
    for gamma, delta in CONFIGS:
        ...
        replacement = rf"\1 & \2 & \3 & \4 & \5 & {fmt_delta(delta_cpu)} & {fmt_delta(delta_gpu)} & {fmt_p(p_val)} \\\\"
