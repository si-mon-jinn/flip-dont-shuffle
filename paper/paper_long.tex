...
\author{Simone Ceppi \and Ignacio Sanchez \\
  ...
  Ispra, Italy \\
  \texttt{\{simone.ceppi, ignacio.sanchez\}@ec.europa.eu}}
...
The key insight is that a counter-based RNG (CBRNG), given a seed and a position, computes the random value at that position in $O(1)$ without computing any preceding values. This allows us to determine the ``greenness'' of any token ID independently, turning a global coordination problem into a local, parallelizable computation. Our implementation uses Philox 4x32-10~\cite{salmon2011parallel}, a CBRNG with strong statistical properties and native GPU support.
...
We replace this with the Bob Jenkins integer hash~\cite{jenkins1997hash}, that compiles into pure ALU instructions with full throughput avoiding scattered memory accesses that thrash GPU caches. The larger output range also has consequences for watermark quality, which we analyze in Section~\ref{sec:hash}.
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_ks_onesample.tex}}
\end{center}
\vspace{-2mm}
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_zscore_wm.tex}}
\end{center}
\vspace{-2mm}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/simple1_combined.pdf}}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/selfhash_combined.pdf}}
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_roc.tex}}
\end{center}
\vspace{-2mm}
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_ppl.tex}}
\end{center}
\vspace{-2mm}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/perplexity_combined.pdf}}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/e2e_overhead_comparison_128tok.pdf}}
...
Our evaluation has limitations that suggest directions for future work. First, all experiments were conducted on a single GPU (RTX 3090) and a single model (Qwen3-8B); generalization to other hardware and model architectures remains to be validated. Second, our robustness analysis is limited: we did not evaluate resistance to paraphrasing attacks, truncation, or other post-hoc modifications; while our theoretical argument for inherited security is grounded in the shared statistical property (Section~\ref{sec:security}), empirical validation under adversarial conditions remains necessary. Third, our GPU implementation relies on compiler-generated Triton kernels via \texttt{torch.compile}; hand-tuned CUDA kernels with explicit memory coalescing and warp-level optimizations could further reduce latency, particularly at small batch sizes where kernel launch overhead dominates. Fourth, while the stateless formulation is architecturally compatible with distributed and multi-device inference (each device can independently compute identical green lists from the shared key), we have not empirically validated this in a tensor-parallel or pipeline-parallel deployment.
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_hash_attribution.tex}}
\end{center}
\vspace{-2mm}
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_null_calibration.tex}}
\end{center}
\vspace{-2mm}
...
\begin{center}
\footnotesize
\resizebox{\columnwidth}{!}{\input{tables/tab_ppl_hash_isolation.tex}}
\end{center}
\vspace{-2mm}
...
\footnotesize
%\resizebox{\columnwidth}{!}{
\input{tables/tab_diversity.tex}
%  }
\end{center}
\vspace{-2mm}
...
\begin{center}
\footnotesize
\input{tables/tab_ks_twosample_nowm.tex}
\end{center}
\vspace{-2mm}
...
\footnotesize
%\resizebox{\columnwidth}{!}{
\input{tables/tab_ks_twosample_wm.tex}
%}
\end{center}
\vspace{-2mm}
...
\textbf{Code.} The full experimental pipeline, including generation scripts, evaluation code, and scripts to compute tables and plots, will be released upon publication.
...
We benchmark against SynthID-Text~\cite{dathathri2024scalable}, vendored from the official \texttt{google-deepmind/synthid-text} repository (Apache 2.0 license). To measure SynthID's \emph{theoretical minimum} latency, we inline all operations into a single \texttt{torch.compile(mode="max-autotune")} function, stripping state management, context history tracking, and safety checks. We verified bit-identical outputs between our compiled pipeline and the reference implementation; tests are available in the paper repository. This gives SynthID every advantage: our reported numbers represent a lower bound on its actual production cost.
...
\item Same \texttt{torch.compile(mode="max-autotune")} for all compiled paths
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/synthid_speedup.pdf}}
\caption{Speedup ratio (log scale). Values above 1.0 indicate slower than the SBW reference.}
\label{fig:synthid_speedup}
\end{figure}

\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/synthid_latency.pdf}}
...
\begin{center}
\footnotesize
\resizebox{\textwidth}{!}{\input{tables/tab_synthid_full.tex}}
\end{center}
\vspace{-2mm}
...
\begin{figure}[!t]
\centerline{\includegraphics[width=\columnwidth]{figures/tail_latency.pdf}}
...
\begin{center}
\footnotesize
\input{tables/tab_tail_latency.tex}
\end{center}
\vspace{-2mm}
